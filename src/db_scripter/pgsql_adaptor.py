...
class PgSqlAdaptor(Adaptor):
    ...
    def generate_insert_script(self, table: Table) -> str:
        ...
        return_id = ""
        autoinc_fields = [f for f in [table.find_field(str(g)) for g in table.fields] if f.auto_increment]
        if len(autoinc_fields) > 0:
            return_id = "returning " + ", ".join([f"{f.name}" for f in autoinc_fields])
        result = f"insert into \"{table.name}\" ({params}) values ({values}) {return_id};"
    ...
    def get_field_type(self, field: Field | UDDT, original_db_type: str) -> str:
        if original_db_type == "pgsql" and field.native_type is not None:
            return field.native_type.name.raw()

        if field.generic_type == "integer":
            if field.size == 1 or field.size == 2:
                return "SMALLINT"
            elif field.size == 4:
                return "INT"
            elif field.size == 8:
